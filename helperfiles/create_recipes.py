...
def validate_recipe(item):
    if (
        (
            "LearnedFromItem" in item["flags"] and item["id"] not in good_recipes
        ...
        or (
            all(x in ["Scribe", "Jeweler"] for x in item["disciplines"])
            and item["min_rating"] >= 400
        )
...
async def get_guild(session, guild_ids):
    guild_items = {}
    guild_sets = (
        ",".join(guild_ids[i : i + 200]) for i in range(0, len(guild_ids), 200)
    )
    jobs = (
        asyncio.ensure_future(
...
def write_recipes(recipes, items):
    ...
    for _recipe, data in list(recipes.items()):
        ...
        for it in data["disciplines"]:
            ...
            if "LearnedFromItem" in data["flags"] and not (
                ...
                continue
            if it == "Chef" and (
                set(good_items) & ingredient_set or "LearnedFromItem" in data["flags"]
            ):
                key = "Chef_karma"
    ...
    for craft in crafts:
        page = [
            "# Created: {} PST".format(datetime.now().strftime("%Y-%m-%dT%H:%M")),
            "recipes = {",
        ]
...
def clean_items(recipes, items):
    ...
    while removed:
        removed = False
        allowed_bound_items = (
            {recipes[x]["output_item_id"] for x in recipes}
            | good_items
            | {x for x in good_vendor}
        )
        for recipe in sorted(recipes):
            for x in recipes[recipe]["ingredients"]:
                if (
                    ...
                    and x["item_id"] not in allowed_bound_items
                ):
                    removed = True
                    del recipes[recipe]
                    break
...
async def itemlist(items, guild, session, lang="en"):
    ...
    if lang == "en":
        ...
        for i in sorted(items):  # otherwise output is semi random order
            ...
            page.append(
                f"\t{i}: {{'output_item_count': {items[i]['output_item_count']}, 'type': '{items[i]['type']}', 'rarity': '{items[i]['rarity']}', 'vendor_value': {items[i]['vendor_value']}, 'flags': {items[i]['flags']}, 'img_url': '{items[i]['icon']}'}},  # {items[i]['name']}"
            )
        page.append("}\n")
        ...
        item_sets = (
            ",".join(item_ids[i : i + 200]) for i in range(0, len(item_ids), 200)
        )
        jobs = (
            asyncio.ensure_future(
